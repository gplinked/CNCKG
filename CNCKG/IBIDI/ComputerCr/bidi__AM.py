...
exp_name = 'am'
# normalized = pd.read_csv('am/am_diff.csv', encoding='utf-8-sig')
normalized = pd.read_csv('am/am_centrality_nodes.csv', encoding='utf-8-sig')
# 创建一个空的DataFrame，用于存储合并后的结果
origin = pd.DataFrame()

# 遍历path中的文件
path =  'output_new/AM/BIDI_output/exp_' + '1'

# 请替换为实际的文件路径
for i in range(1, 11):  # 假设文件名分别为origin-1.csv到origin-10.csv
    filename =  "/Difficult_result_" + '15.0' + "_" + str(i) + ".csv"
...
# 检查是否存在重复的proxy
duplicates = merged[merged.duplicated(subset='proxy')]

# 打印重复的proxy
if len(duplicates) > 0:
    print("存在重复的proxy:")
    print(duplicates['proxy'])
else:
    print("没有重复的proxy")
    print(len(merged))
#
merged = merged.merge(normalized[['proxy', 'difficult']], on='proxy', how='left')
#
