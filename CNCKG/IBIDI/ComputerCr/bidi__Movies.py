...
exp_name = 'movies'
# normalized = pd.read_csv('movies/movies_centrality_nodes0.1.csv', encoding='utf-8-sig')
normalized = pd.read_csv('movies/movies_centrality_nodes.csv', encoding='utf-8-sig')

# 创建一个空的DataFrame，用于存储合并后的结果
origin = pd.DataFrame()

# 遍历path中的文件
path =  'output_new/Movies/BIDI_output/exp_' + '1'

# 请替换为实际的文件路径
for i in range(1, 11):  # 假设文件名分别为origin-1.csv到origin-10.csv
    print(i)
    filename =  "/Difficult_result_" + '12.0' + "_" + str(i) + ".csv"
...
# 使用groupby和agg方法来合并重复的DBpedia_URL数据
# 对于detail列使用自定义的determine_detail函数
# 对于sign, difficult, predict, loss列计算均值
aggregated = origin.groupby('DBpedia_URL').agg({
    'sign': 'mean',
    'difficult': 'mean',
    'predict': 'mean',
    'loss': 'mean',
...
# 检查是否存在重复的DBpedia_URL
duplicates = merged[merged.duplicated(subset='DBpedia_URL')]

# 打印重复的DBpedia_URL
if len(duplicates) > 0:
    print("存在重复的DBpedia_URL:")
    print(duplicates['DBpedia_URL'])
else:
    print("没有重复的DBpedia_URL")
    print(len(merged))
#
merged = merged.merge(normalized[['DBpedia_URL', 'difficult']], on='DBpedia_URL', how='left')
#

# 存储合并后的结果到指定路径
output_path = 'output-2/movies/acc951'  # 请替换为实际的输出路径
...
# 定义10个等间隔的bins
bins = [i/20.0 for i in range(21)]  # 生成0, 0.1, 0.2, ..., 1.0 的列表
