...
exp_name = 'bgs'
normalized = pd.read_csv('bgs/bgs_centrality_nodes.csv', encoding='utf-8-sig')

# 创建一个空的DataFrame，用于存储合并后的结果
origin = pd.DataFrame()

# 遍历path中的文件
path =  'output_new/BGS/BIDI_output/exp_' + '1'

# 请替换为实际的文件路径
for i in range(1, 11):  # 假设文件名分别为origin-1.csv到origin-10.csv
    print(i)
    filename =  "/Difficult_result_" + '6.0' + "_" + str(i) + ".csv"
...
# 检查是否存在重复的rock
duplicates = merged[merged.duplicated(subset='rock')]

# 打印重复的rock
if len(duplicates) > 0:
    print("存在重复的rock:")
    print(duplicates['rock'])
else:
    print("没有重复的rock")
    print(len(merged))
#
merged = merged.merge(normalized[['rock', 'difficult']], on='rock', how='left')
#
